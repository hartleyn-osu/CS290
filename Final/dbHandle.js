...
app.get('/insert', function(req, res, next){
	...
	mysql.pool.query("INSERT INTO exercise (`name`) VALUES (?), 
		(`reps`) VALUES (?), (`weight`) VALUES (?), 
		(`pounds`) VALUES (?), (`exDate`) VALUES (?)", *fix*[req.query.c], function(err, result){
			if(err){
				next(err);
				return;
			}
...
app.get('/simple-update', function(req, res, next){
	var context = {};
	mysql.pool.query("UPDATE exercise SET name=?, reps=?, weight=?,
		pounds=?, exDate=? ", 
	...
	mysql.pool.query("SELECT * FROM exercise WHERE id=?", 
		[req.query.id], function(err, result){
			...
			if(result.length == 1){
				...
				mysql.pool.query("UPDATE exercise SET name=?, 
					reps=?, weight=?,
					pounds=?, exDate=? WHERE id=? ",
